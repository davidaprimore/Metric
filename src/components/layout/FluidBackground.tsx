...
type FluidVariant = 'professional' | 'patient' | 'auth' | 'luminous' | 'light' | 'marble';
...
export const FluidBackground: React.FC<FluidBackgroundProps> = ({
    ...
    const themes = {
        ...
        marble: {
            bg: 'bg-[#EAEEF2] bg-[radial-gradient(circle_at_0%_0%,_rgba(203,213,225,0.5),_transparent_50%),_radial-gradient(circle_at_100%_100%,_rgba(203,213,225,0.5),_transparent_50%)]', // Slightly Darker Ice
            noise: 'opacity-[0.6]'
        }
    };
    ...
    return (
        <div className={`relative min-h-screen w-full transition-colors duration-700 ${className}`}>
            ...
            <div className={`fixed inset-0 w-full h-full overflow-hidden pointer-events-none z-0 ${['light', 'marble'].includes(variant) ? theme.bg : 'bg-[#0A1A2F]'}`}>

                {!['light', 'marble'].includes(variant) && (
                    <>
                ...
                {/* MARBLE SPECIAL: Sunlight Beam */}
                {variant === 'marble' && (
                    <div className="absolute inset-0 w-full h-full pointer-events-none z-0">
                        {/* Warm Sunlight from Top Right */}
                        <div className="absolute top-0 right-0 w-[90%] h-[90%] bg-[radial-gradient(circle_at_100%_0%,_rgba(251,191,36,0.12),_transparent_50%)]"></div>

                        {/* Secondary Ambient Glow */}
                        <div className="absolute top-[20%] right-[10%] w-[60%] h-[60%] bg-amber-500/05 blur-3xl rounded-full"></div>
                    </div>
                )}
